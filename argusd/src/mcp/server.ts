...
const TOOLS = [
  { name: "browser_open", description: "Open a URL in a lane and return what loaded plus an outline of the page (landmarks, headings, fields, actions with refs, and which actions are covered or disabled). Start here.",
    inputSchema: { type: "object", properties: { url: { type: "string" }, lane, viewport: { type: "object", description: "{w, h}; widths under 600 emulate a phone", properties: { w: { type: "integer" }, h: { type: "integer" } } } }, required: ["url"] } },
...
export class McpServer {
  ...
  private async laneId(name: string | undefined, open?: { url: string; viewport?: { w: number; h: number } }): Promise<{ id: string; opened?: Record<string, unknown> } | null> {
    ...
    if (!open) return null;
    const opened = await c.call<Record<string, unknown>>("lane.open", { kind: "throwaway", url: open.url, label, ...(open.viewport ? { viewport: open.viewport } : {}) });
  ...
  async callTool(name: string, args: Record<string, unknown>): Promise<ToolResult> {
    ...
    switch (name) {
      case "browser_open": {
        ...
        const found = await this.laneId(args.lane as string | undefined, { url: args.url as string, ...(viewport ? { viewport } : {}) });
        const c = await this.daemon();
        let head: string[];
        if (found!.opened) {
          head = [`lane ${(args.lane as string) || "main"} opened`, ...renderObservation((found!.opened.observation ?? {}) as never)];
        } else {
          const steps: unknown[] = [];
